...
export const ProductPageAdmin: FC<Props> = ({ product }) => {
  ...
  const onSuccess = (product: IProduct) => {
    if (product) {
      // Con replace no se puede volver a la pagina anterior (la de creacion de producto)
      // replace para sustituir la pagina actual (la de creacion de productos) por la del detalle del producto
      router.replace(`/admin/products/${product.slug}`)
      // Con push se puede volver a la pagina anterior (la de creacion de productos)
  ...
  const onSubmit = async (form: FormData) => {
    ...
    //  REEMPLAZADO en el metodo onSuccess
    //     console.log({data});
    //     if ( !form._id ) {
    ...
    //  REEMPLAZADO en dentro del if : if(createProduct.error)
    // } catch (error) {
    //     console.log(error);
    //     setIsSaving(false);
    // }
